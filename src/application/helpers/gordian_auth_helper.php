...
if (!function_exists('gordian_auth_user_widget'))
{
	function gordian_auth_user_widget($config = array())
	{
		 ...
		 * Email Field Details
		 */
		$email_data = (array_key_exists('email_label', $config))
			? $config['email_label'] 
		 ...
		 */
		$nickname_data = (array_key_exists('nickname_label', $config))
			? $config['nickname_label'] 
		 ...
		 */
		$password_data = (array_key_exists('password_label', $config))
		 ...
		 */
		$confirm_data = (array_key_exists('confirm_label', $config))
		 ...
		 */
		$register_label = (array_key_exists('register', $config))
			? $config['register'] 
		 ...
		 * Register new account link
		 */
		$login_label = (array_key_exists('login', $config))
			? $config['login'] 
		 ...
		 */
		$forgotten_label = (array_key_exists('forgotten', $config)) 
			? $config['forgotten'] 
		 ...
		 */
		$LABEL_SUFFIX = $CI->lang->line('gordian_localized_label_suffix');
		...
		if (!(array_key_exists('nickname', $config) && $config['nickname'] !== FALSE))
		{
			...
			echo '		<td align="right">' . $nickname_field . "<br />";
		...
		echo '		<td align="right">' . $password_field. '</td>';
		echo '	</tr>';
		...
		if (!(array_key_exists('confirm', $config) && $config['confirm'] !== FALSE))
		{
			...
			echo '		<td align="right">' . $confirm_field . '</td>';
			echo '	</tr>';
		}
		...
		if (!(array_key_exists('register', $config) && $config['register'] !== FALSE))
		{
			echo $register_link;		
		}
		...
		if (!(array_key_exists('login', $config) && $config['login'] !== FALSE))
		{
			echo $login_link;		
		}
		...
		if (!(array_key_exists('forgot', $config) && $config['forgot'] !== FALSE))
		{
			echo $forgotten_link;
		}
